...
- `va-*` agents
- `sdlc-init` / `sdlc-load` / `session-start` / `session-end` prompts
...
在已有项目根目录的 Copilot Chat 中使用：

- `/sdlc-load`

该命令会自动识别项目是否已经接入 SDLC：

- 已完整接入：直接恢复上下文并继续开发
- 部分接入：只补齐缺失的 SDLC 骨架，再继续开发
- 尚未接入的旧项目：在不覆盖现有业务代码的前提下完成纳管，再继续开发
